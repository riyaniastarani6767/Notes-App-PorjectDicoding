// /* ================== VERSION & CACHES ================== */
// const VERSION = "v1.1.3"; // bump supaya SW pasti update
...
/* ================== VERSION & CACHES ================== */
const VERSION = "v1.1.4"; // bump supaya SW pasti update di production
const STATIC_CACHE = `static-${VERSION}`;
const RUNTIME_CACHE = `runtime-${VERSION}`;
...
self.addEventListener("fetch", (event) => {
  ...
  const url = new URL(req.url);
  if (!url.protocol.startsWith("http")) return; // skip chrome-extension:, data:, etc.
...
/* ================== PUSH NOTIFICATIONS ================== */
// util aman untuk menampilkan notif
async function safeShowNotification(title, options) {
  if (Notification.permission !== "granted") return; // tanpa izin, jangan tampilkan
  try {
    await self.registration.showNotification(title, options);
  } catch (err) {
    // jangan biarkan error ini “Uncaught (in promise)”
...
self.addEventListener("message", (e) => {
  if (e.data && e.data.type === "test-notif") {
    ...
    e.waitUntil(safeShowNotification(title, options));
  }
});
