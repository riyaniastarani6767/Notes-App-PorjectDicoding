...
export default {
  ...
  async afterRender() {
    ...
    // --- Render daftar API (tambah createdAt sebagai teks ke-3) ---
    listEl.innerHTML = stories
      .map((s) => {
        const title = s.name ?? s.author ?? "-";
        const desc = s.description ?? s.desc ?? "-";
        ...
        // [ADD] ambil createdAt dari berbagai kemungkinan field & format
        const createdRaw =
          s.createdAt ?? s.created_at ?? s.created ?? s.date ?? null;
